  ...
  <div class="container">
    ...
    <form>
      ...
      <div class="input-group">
        <span class="input-group-addon" style="width:100px; color:red;">手工單號碼</span>
      ...
      <div class="input-group">
        <span class="input-group-addon" style="width:100px;">倉庫</span>
          <select class="form-control required" v-model="line.SUBINVENTORY" style="background-color: #FFFF99; font-size:17px;"
            ref="line.SUBINVENTORY" @keydown.enter="update()" data-vv-name="倉庫">
        ...
        <div class="" align="center" nowrap style="margin-top:20px;">
          <span style="margin:5px;">
            <button class="font-big btn btn-danger" type="button"
              style="width:120px; height:50px; font-size:20px;" @click="$router.push({ name: 'IMPS03003_2', params: {items: items, item: item, ignoreInsert: 'Y', header: header, PO_NUM: header.PO_NUM, isPO: $route.params.isPO} })">
              取消
            </button>
          </span>
          <span style="margin:5px;">
...
export default {
  ...
  methods: {
    modify: function () {
      ...
      fetch(this.domain + '/WIPF/IMPS_API.do?reqCode=imps03005Modify', {
        method: 'POST',
        ...
        .then(data => {
          ...
          if (data.result === 'Y') {
            ...
            this.$router.push({ name: 'IMPS03003_2', params: {items: this.$data.items, item: this.$data.item, ignoreInsert: 'Y', header: this.$data.header, PO_NUM: this.$data.header.PO_NUM, isPO: this.$route.params.isPO} })
          } else {
    ...
    init: function () {
      this.$parent.showLoader()
      fetch(this.domain + '/WIPF/IMPS_API.do?reqCode=imps03005Init', {
        method: 'POST',
        body: JSON.stringify(this.$data)
      })
        .then(response => response.json())
        .then(data => {
          ...
          if (data.result === 'Y') {
            ...
            if (data.SHIPMENT_NUM !== undefined) {
              this.header.SHIPMENT_NUM = data.SHIPMENT_NUM
            }
    ...
    showDialog: function () {
      ...
      let message = {
        title: '修改確認',
        body: '倉庫:' + vm.line.SUBINVENTORY + '  數量:' + vm.line.REAL_QTY
      }
